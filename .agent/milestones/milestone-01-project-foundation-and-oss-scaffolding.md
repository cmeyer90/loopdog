...
> is loopdog's own repo hygiene, the floor everything else builds on.

## Objective

Stand up loopdog as a healthy open-source project: pick the implementation stack
and repo layout, add the licensing and community files, and make loopdog's own CI
+ branch protection trustworthy so the tool is itself built to the standard it
enforces on adopters.

## Guiding Decisions

- Loopdog builds to the standard it enforces: protected CI, CODEOWNERS on its own
  workflow/identity paths, semver from day one.
- **Stack: TypeScript on Node 20+, an npm-workspaces monorepo** of small
  `@loopdog/*` packages — chosen for the GitHub-App/Octokit/Actions ecosystem. The
  module boundaries, filetree, and build order are defined in
  [`docs/codebase.md`](../../docs/codebase.md); 0001 scaffolds that skeleton.
- **Modular by construction:** clean package boundaries (ports in `@loopdog/core`,
...
|---:|---|---|---|---|
| 0001 | verified | task/0001-stack-and-repo-layout | Stack & Repo Layout | The `@loopdog/*` workspace skeleton per `docs/codebase.md` (8 packages, building green) + a runnable `loopdog --help`. |
...
| 0003 | verified | task/0003-own-ci-pipeline | Loopdog's Own CI | Green, reproducible CI (lint + test + build) on every PR. |
...
- [x] The repo has the `@loopdog/*` workspace skeleton matching `docs/codebase.md`
...
- [~] CI is green and reproducible (locally + workflow added; first live Actions
  ...
  0004); CODEOWNERS gates loopdog's own workflow/identity files.
- [x] A versioning + release process exists (changesets; ships pre-1.0 builds).

## Verification Log

- 2026-06-09: 0001 verified — `npm run build`/`test`/`lint` + `loopdog --help`
