...
> Background: [Loopdog Architecture](../../docs/architecture.md) — "Identity &
...
Give loopdog a scoped repo identity and a two-plane secret model: a **provider auth
...
- **Loopdog's repo identity = the Actions `GITHUB_TOKEN`** — free, zero-setup. The
  loop-to-loop handoffs it won't re-trigger are carried by the **cron sweep**
  (M02 · 0076), so **no loopdog GitHub App is required for V1** (optional PAT for
  instant handoff; a full loopdog App is post-V1). Local CLI auth is OAuth device
...
- Project secrets for the work cell live in the **provider's** cloud env (setup
  ...
  them in Claude's cloud environment and Loopdog does not forward Actions secrets
...
|---:|---|---|---|---|
| 0029 | verified | task/0029-provider-auth-and-scoped-identity | Repo Identity & Provider Auth | Repo identity = Actions `GITHUB_TOKEN` + cron sweep for handoff (optional PAT for instant); local `loopdog login` via OAuth device flow / `gh`; provider subscription via Claude routine import / provider App. No loopdog GitHub App, no manual PATs required. |
...
- [x] Repo identity = Actions `GITHUB_TOKEN` (sweep handoffs; optional
  LOOPDOG_PAT for instant; fork-readonly defer) — no loopdog GitHub App; provider
