...
**block** the item into `loopdog:state/needs-clarification` — biased hard toward
...
`@loopdog/core` (pure, IO-free); config schema in `@loopdog/config`; the runtime
...
**Config** (`@loopdog/config`; repo-wide in `loopdog.yml`, override per loop):
...
- `assumptions` → rendered into a `<!-- loopdog:assumptions -->` block on the issue
...
- `blockers` → rendered into a `needs-clarification` comment listing each open
  question with its category; the runtime sets `loopdog:state/needs-clarification`.
...
- [x] Define `OpenQuestion`, `QuestionCategory`, `GroomingDecision` in `@loopdog/core`.
- [x] Implement `classifyQuestion` + `decideGrooming` (pure; reasons on every decision).
- [x] Add `grooming.clarification` to the `@loopdog/config` zod schema with defaults.
- [x] Render the `<!-- loopdog:assumptions -->` block + the clarification-comment
...
Tests run via the repo's `vitest` runner; this is pure `@loopdog/core` logic so the
