...
Make every brief looper dispatches come from **versioned, reviewable repo files**
...
consumes. No prompt text lives inline in looper's code.
...
prompts ship as `templates/loops/<name>/prompt.md` assets in `@looper/runtime`;
adopters override them in their own repo. Lands in `@looper/backends`
(`src/brief/`), reading config from `@looper/config` (0006).
...
- Shared **policy fragments** (`.looper/policies/*.md`) referenced by name so
  common guidance (output contract, secret-hygiene, style) is authored once and
  reused across loops.
- A `looper prompts` surface (show/diff/lint) so operators can see exactly what
...
```
.looper/loops/<loop>/prompt.md              # the loop brief (overrides built-in)
.looper/loops/<loop>/prompt.<backend>.md    # optional per-backend overlay (claude|codex|self-hosted)
.looper/policies/<name>.md                  # shared fragments (output-contract, secret-hygiene, …)
templates/loops/<loop>/prompt.md            # built-in default, shipped by @looper/runtime
```
...
**Types (`@looper/backends/src/brief/`).**
```ts
...
type ComposeContext = {                 // supplied by the runner (0012)
  ...
  acceptanceCriteria: string;           // the looper:acceptance-criteria block (0014)
  transition: { from: string; to: string };
  runId: string; loop: string; backend: BackendId;
  branch: string;                       // looper/<loop>/<issue>-<run_id> (0073)
...
type Brief = {                          // the object 0019 dispatch() receives
  ...
  outputContract: string;               // "open a PR labeled in-review, branch …, trailer looper-run: …"
...
adopter `.looper/loops/<loop>/prompt.md` → `prompt.<backend>.md` overlay. The
...
`looper/<loop>/<issue>-<run_id>`, put `looper-run: <run_id>` in the PR body, label
...
**CLI (`@looper/cli`, thin; thread through 0069/0078).**
`looper prompts show <loop> [--backend b] [--issue N]` → prints the exact brief;
`looper prompts diff <loop>` → built-in vs. adopter override; `looper prompts lint`
...
- The `looper loops new` questionnaire that scaffolds the folder (M16 · 0078).
...
npm test -w @looper/backends      # composer purity, resolution order, ref stability
npm test -w @looper/cli           # prompts show/diff/lint over fixtures
