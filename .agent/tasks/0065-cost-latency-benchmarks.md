...
numbers** — a reproducible `loopdog bench` command that folds the existing
...
and the milestone gates `1.0.0` on proving loopdog on a real external repo. This
...
- A pure **benchmark projection** in `@loopdog/core` that turns a `ProviderLedger`
...
- A `loopdog bench` CLI command (`@loopdog/cli`) that reads the telemetry ledger,
...
**`loopdog bench` (`@loopdog/cli`):**

```
loopdog bench [--since 30d] [--loop implement] [--backend claude]
...
`loopdog bench --out docs/benchmarks.md`; the surrounding prose is preserved by
writing only between stable `<!-- loopdog:benchmarks -->` … `<!-- /loopdog:benchmarks -->`
markers.
...
representative window, let 0053 persist records to the `loopdog/telemetry` branch,
then `loopdog bench --since <window>` against that repo. No quota is spent by the
...
- [x] `loopdog bench` renders the report to stdout (md default) and to a file via
...
- [x] `docs/benchmarks.md` exists, contains the rendered table between the
      `loopdog:benchmarks` markers plus a Methodology section, and is regenerable by
      `loopdog bench --out docs/benchmarks.md` without clobbering the prose.
- [~] Published numbers sourced from the 0063 dogfood telemetry — OPERATOR-PENDING
      ...
      methodology + a placeholder; `loopdog bench --out docs/benchmarks.md` fills
...
- [x] Add `BenchRow`/`BenchmarkReport` + `projectBenchmark`/`renderBenchmarkMd` in
      `@loopdog/core` (`core/src/run-record/benchmark.ts`), re-exported from the barrel.
- [x] Implement `loopdog bench` in `@loopdog/cli` over `TelemetrySink.ledger/query`,
      with `--since/--loop/--backend/--format/--out` and marker-bounded file write.
- [x] Add a deterministic baseline scenario in `@loopdog/testing` (recorded cassette
...
#   loopdog bench --format json → report matches the fixture ledger.
#   loopdog bench --out tmp.md  → writes only between the markers, preserves prose;
...
- 2026-06-12: `projectBenchmark` + `renderBenchmarkMarkdown` green
  ...
  (no NaN); `--since`/`--loop`/`--backend` filters apply. `loopdog bench` (CLI,
  read-only) renders md/json to stdout or `--out`, splicing between the
  `loopdog:benchmarks` markers in `docs/benchmarks.md` without clobbering the
...
- `BenchmarkReport` JSON is the stable published contract (one row per loop+
  ...
  marker-bounded (`<!-- loopdog:benchmarks -->`) so `loopdog bench --out` regenerates
...
`loopdog bench` renders it (md/json, stdout or `--out`), splicing into
