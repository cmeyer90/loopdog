...
loopdog degrades — and the terminal behaviors they drive: quarantine for poisoned
...
- The `resilience:` config block (root default + per-loop override), validated by
  M02 · 0006.
- `loopdog:quarantine` for items that exhaust `max_attempts_per_item`, with the
  failure recorded in the run record.
- `on_failure` routing (`needs-human` | `retry` | `abandon`) and `escalate_to`
  notification.
- CLI visibility: quarantined/escalated items in `loopdog status`; a release path.
...
resilience:
  ...
  max_attempts_per_item: 3        # exhausted → loopdog:quarantine + on_failure
...
- **Quarantine** is a terminal hold: the item gets `loopdog:quarantine`, its run
  record captures the failure class + last error, and it leaves the active pipeline
  until a human clears it (`loopdog retry <item>` re-queues; clearing is audited).
- **Escalation** assigns/pings `escalate_to` and comments the reason.
- The CLI surfaces `loopdog status` counts (quarantined/escalated/paused) and a
  `loopdog retry`/`loopdog resume` path; ties into M16.
...
- [x] An item exhausting `max_attempts_per_item` is quarantined with its failure
      recorded — never silently dropped — and is human-releasable (`loopdog retry`).
- [x] `on_failure` routing and `escalate_to` notification work.
- [x] `loopdog status` surfaces quarantined / escalated / approval-held items under
...
- [x] Define + validate the `resilience:` schema (`resilienceSchema` in
      `@loopdog/config`, root + per-loop `.partial()`; backoff enum extended to the
      three 0089 shapes) → resolved to `LoopDefinition['resilience']` + normalized
      to ms policy types by the core `resilience/normalize.ts`.
- [x] Implement quarantine (`loopdog:quarantine` + `loopdog:needs-human` label,
      run-record `escalated`/`poisoned` capture, human release via `loopdog retry`).
- [x] Implement `on_failure` routing (needs-human → quarantine, abandon →
      `loopdog:abandoned`, retry → keep backing off) + `escalate_to` ping in the
      quarantine/timeout comments.
- [x] Surface state in the CLI: `status` ATTENTION list + a new `loopdog retry`
...
# exhaust attempts → quarantined + recorded; loopdog retry → re-queued; escalate pings
```
...
- 2026-06-12: quarantine + escalation e2e green (`resilience-e2e.test.ts`): with
  ...
  backend, the 2nd failed attempt adds `loopdog:quarantine` + `loopdog:needs-human`,
  posts a comment pinging `@team/oncall` + mentioning `loopdog retry`, and writes an
  ...
  + the e2e scaffold edits). `loopdog status` ATTENTION includes `loopdog:quarantine`
  + `loopdog:needs-approval`. Full suite (242) green.

## Decisions

- The `resilience:` schema (durations as `30s`/`10m`/`1h` strings, backoff ∈
  {exponential, linear, constant}) lives in `@loopdog/config` and was scaffolded by
...
- Quarantine capture: the poisoned item gets `loopdog:quarantine` + `loopdog:needs-
  human`, a comment (with the `escalate_to` ping + the `loopdog retry` hint), and an
  ...
  quarantine; `abandon` → `loopdog:abandoned` off-ramp; `retry` → keep backing off
  (no quarantine). The same routing applies on a dispatch-timeout poisoning (0089).
- Release/audit: `loopdog retry <item>` removes `loopdog:quarantine` + `loopdog:needs-
  human` + the `loopdog:attempts/*` + `loopdog:not-before/*` labels and posts an
  auditable "released by <who>" comment, so the sweep re-attempts cleanly. (Like
  `loopdog approve`, it acts over the real `GitHubPort` — covered by the runtime
...
(`loopdog:quarantine` + `needs-human`) with its failure recorded and an
`escalate_to` ping — never silently dropped — and is human-releasable with
`loopdog retry`. `on_failure` routes needs-human/abandon/retry; `loopdog status`
