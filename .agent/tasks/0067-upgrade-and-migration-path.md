...
Give adopters a safe, deterministic way to move an attached repo from one looper
release to the next: a versioned config contract, a `looper upgrade` command that
applies ordered, idempotent migrations to the scaffolded `.looper/` tree and
...
config (`version:`) and keep an upgrade path (M15 · 0067)"). Because looper is
...
lands mostly in `@looper/config` (the version contract + migration registry) and
`@looper/cli` (`commands/upgrade.ts`), reusing the `templates/` resolution and
create/skip/conflict merge from `looper init` (0007). See
...
- A **version contract**: the root `looper.yml` `version:` is the single schema
  version for the whole `.looper/` tree; the installed looper package declares the
...
- `looper upgrade`: detect installed-vs-on-disk version, compute the migration
...
- A **compatibility pre-flight** in the runtime: a controller refuses to run
  ...
  migration floor — fail closed with a "run `looper upgrade`" message.
...
package exposes, in `@looper/config`:
...
```ts
interface ConfigTree {                  // in-memory snapshot of .looper/ + workflow callers
  root: unknown;                        // parsed looper.yml
  loops: Record<string, { loop: unknown; prompt: string }>;
  workflows: Record<string, string>;    // .github/workflows/looper-*.yml
}
interface Migration {
...
**`looper upgrade` flow** (in `@looper/cli/commands/upgrade.ts`, mirroring 0007):
...
4. **Apply**: fold the chain over the snapshot, then write changed files. A file
   ...
   silently overwritten — emit a `.looper/<file>.looper-new` alongside it and list
   it for manual merge (so adopter prompt edits survive an upgrade). Workflow
   callers (`looper-events.yml`, `looper-sweep.yml`) are bumped to the new `uses:`
   ref version.
5. **Validate**: run `@looper/config` validation (0006) on the result; fail closed.
...
(the `@looper/runtime` composition root, alongside the existing pre-flight gates)
...
"controller vX needs `looper upgrade`" error and a non-zero exit, **before** any
...
summary nudging `looper upgrade`.

**Versioning policy.** Tie `CONFIG_VERSION` bumps to semver: a config-breaking
change ⇒ a new migration + a major (or pre-1.0 minor) looper release; every such
...
is idempotent; `--path` not a looper tree → error; a conflict on a workflow caller
→ still write the new ref (workflows are looper-owned, not adopter-edited) but
report it.

## Out Of Scope

- The config schema/validator itself and its fields (0006); `looper init`
  ...
  their CI (looper only migrates the `.looper/` tree + its workflow callers).
...
# adopter-edited prompt.md → conflict preserved as .looper-new, original untouched
...
A buggy migration could corrupt an adopter's `.looper/` tree — mitigated by:
