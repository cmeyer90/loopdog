...
so loopdog can build/test/lint/run/deploy any repo on day one without hand-written
...
- Explicit-override path: when `loopdog.yml` names an adapter (or per-loop config
  does), detection is skipped/annotated, never silently overridden.
- Surfacing the result so `loopdog init` / `loopdog status` (M16) can show "detected
  Node (pnpm)" with its evidence, and so detection re-runs are reproducible.

### Technical detail

Lands in **`@loopdog/adapters`** (`packages/adapters/src/detect/`). Detection is a
...
with `evidence: ["explicit override in loopdog.yml"]` (still attach detection as
advisory); else top match if `confidence >= FLOOR` (default `0.5`, a `loopdog.yml`
...
Config keys (root `loopdog.yml`, validated in `@loopdog/config`):
...
- [x] An explicit `adapter:` in `loopdog.yml` is honored verbatim; detection still
...
- [x] Consume `RepoFs` (declared in `@loopdog/core` per 0024), define `DetectionMatch`,
...
- [x] Add `adapter` / `adapters.detect.*` keys to the `@loopdog/config` schema.
- [x] Surface the detection result for `loopdog init`/`status` (M16).
...
always advisory and overridable in `loopdog.yml`; the result is surfaced with
evidence so an adopter can correct it during `loopdog init`. Rollback is config-only
