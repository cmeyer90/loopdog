...
issue's `loopdog:state/*` label and the plan `Status` from ever disagreeing.
...
acceptance-criteria block lives in the bound task file. Lands in `@loopdog/plans`
(generator + binding + mirror) with the `PlanStore` interface in `@loopdog/core`;
the effectful sync step is called from the `@loopdog/runtime` transition pipeline.
...
- A **label ↔ Status mirror**: a pure mapping between `loopdog:state/*` labels and
...
`<!-- loopdog:acceptance-criteria -->` block (0014) carried verbatim, sets
`Status: planned`, and writes `<store>/tasks/NNNN-<slug>.md`. An **epic** (issue
labeled `loopdog:epic` or referencing children) also gets a milestone via the
...
1. **Issue → plan:** a fenced marker appended to the issue body, parseable from
   GitHub state alone:
   ```
   <!-- loopdog:plan task=0016 milestone=04 path=.agent/tasks/0016-issue-to-plan-binding.md -->
   ```
...
**Label ↔ Status mirror.** A single pure table in `@loopdog/core` (next to the state
machine, 0011) is the only place the mapping lives:

```
loopdog:state/new,needs-grooming,needs-clarification  -> planned
loopdog:state/ready-for-agent                         -> ready
loopdog:state/in-progress                             -> in-progress
loopdog:state/in-review,changes-requested             -> implemented
loopdog:state/verified                                -> verified
loopdog:state/merged                                  -> merged
loopdog:state/deployed                                -> merged   (deploy ≠ a plan status)
loopdog:state/deploying,loopdog:state/deploy-failed,loopdog:state/rolled-back -> merged
loopdog:blocked,loopdog:needs-human,loopdog:stuck       -> blocked
...
Operational hold labels such as `loopdog:needs-approval`, `loopdog:approved`,
`loopdog:parked`, `loopdog:stop`, and claim/pause markers are orthogonal: they do
...
- [x] The issue carries a `<!-- loopdog:plan … -->` marker and the task file carries
...
- [x] `@loopdog/core` stays IO-free (mapping + types only; writes go through
...
- [x] Implement `bindIssue` (id alloc via 0015, template render, marker + `Issue:`
      field, epic→milestone) in `@loopdog/plans`.
...
- [x] Wire the bind + reconcile step into the `@loopdog/runtime` pipeline so a
...
npm test -w @loopdog/core   # mapping totality + table
...
- Marker format as specced: `<!-- loopdog:plan task=NNNN path=… -->` on the
...
`@loopdog/plans/binding`: bindIssue (idempotent generator from the 0015
