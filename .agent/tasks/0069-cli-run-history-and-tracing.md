# 0069 Run History & Tracing (`loopdog runs list` / `loopdog runs show`)
...
Let an operator trace any loop run from the CLI: `loopdog runs list` to browse
history, and `loopdog runs show <run>` to see exactly **what ran, how it was
...
loops auditable — the answer to "what did loopdog do on this issue, and why." It
...
- `loopdog runs list` — table of recent runs. Flags `--loop <l>`, `--item <#>`,
  `--status <s>`, `--since <when>`, `--limit <n>`, `--json`.
- `loopdog runs show <run>` — full trace (see Command Surface). Flags `--brief`
...
```
$ loopdog runs list --loop implement --limit 3
...
Run run_91c   loop: implement   item: #142 "Add rate limiting to the public API"
  ...
  Dispatched brief:                          # HOW IT WAS PROMPTED (composed for this run)
    ...
    └ full: loopdog runs show run_91c --brief
  ...
  Artifacts: PR #143 · plan .loopdog/plans/tasks/0001-api-rate-limiting.md · gh run 1234
```

## Out Of Scope

- Live tailing of an in-progress run (0070 `loopdog tail`).
- Loop-level (not run-level) introspection (0068).

## Acceptance Criteria

- [x] `loopdog runs list` filters by loop/item/status/since with a `--limit`;
      `--json` mirrors columns.
- [x] `loopdog runs show <run>` shows backend + provider session link, trigger,
...
# loopdog runs show run_91c --json | jq '.steps' ; loopdog runs show run_91c --brief
```
...
`loopdog runs list/show/stats` read the run-record ledger from the
loopdog/telemetry orphan branch (TelemetryBranchStore.readDay over a --since
...
`loopdog runs list/show/stats` trace runs from the durable ledger: filterable
