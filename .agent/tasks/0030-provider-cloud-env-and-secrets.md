...
define how loopdog declares the provider sandbox **setup script + env vars** for
...
than injected by Loopdog at `/fire` time, that Codex strips secrets before the
...
lives in `@loopdog/core` ([codebase](../../docs/codebase.md) — ports table); the
provider-env impls land in `@loopdog/backends`, the config schema in
`@loopdog/config`. This is the env half of the plane; secret *injection + leak
...
environment. Loopdog tracks the required names and validates/document the setup; it
does not forward GitHub Actions secrets into Claude on each fire.

## Scope

- A `loopdog.yml` config block declaring the work-cell environment: a `setup`
...
**Config (lands in `@loopdog/config`, schema validated with `zod`).** A root
`loopdog.yml` `work_cell.env` block, overridable per loop in
`.loopdog/loops/<name>/loop.yml`:

```yaml
work_cell:
  setup: ".loopdog/setup.sh"          # expected setup script / provider setup reference
  env:
  ...
  backends:
    ...
    codex:  { setup: ".loopdog/setup.codex.sh" }   # backend-specific override
```
...
**Resolver (lands in `@loopdog/backends`, a `ProviderEnvResolver` keyed off the
`SecretBackend` port in `@loopdog/core`).** `resolve(workCellEnv, backend) ->
...
2. For backends that support runtime forwarding, read each entry's value from its
   ...
   time — loopdog never holds a long-lived model API key; these are *project*
...
**Validation (wired into `loopdog loops validate` / `doctor`, M16).**
...
- Any Claude routine loop using `from_env` / `from_actions_secret` for a value the
  ...
  environment, keep it CI-only, or switch this loop to self-hosted." Loopdog does
...
- [x] `work_cell.env` (+ `setup`, per-backend overrides) is a validated
      `@loopdog/config` schema with `value` / `from_env` / `from_actions_secret` /
...
- [x] Add the `work_cell.env`/`setup`/`backends.*` schema + validation in
      `@loopdog/config`.
- [x] Implement `ProviderEnvResolver` in `@loopdog/backends` over the `core`
...
- [x] Wire the `sensitive`-against-provider warning + unset-reference fail-loud
      into `loopdog loops validate` / `doctor`.
