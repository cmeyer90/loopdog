# Looper Task-Spec Consistency Review
...
- **Run-record store (H1, N1):** canonical store is the orphan **`looper/telemetry`**
  branch (append-only day-bucketed NDJSON, 0053-owned); `.looper/runs/` removed —
  loop runs never touch the adopter's working branch/PRs.
- **Mode promotion (H12):** one guarded command **`looper promote <loop> --to`**;
  `looper loops set … mode=` removed (kept for non-mode fields).
...
- **Config (H8, H9, M1):** root default at `backends.default`; `0050`'s
  ...
  dropped.
- Plus: `trigger_modes` += `self_hosted_dispatch` (H6); `looper:needs-human` flat
  label (H7); trailing-parens criteria rows + single `looper:plan` attribute tag
...
contracts (the adapter port, `looper.yml` config keys, the run-record store, and
...
**Files:** `0094`, `0053`. `0094` (store owner) → files at `.looper/runs/<run_id>`
on the working branch; `0053` declares its own canonical store: orphan branch
`looper/telemetry` with `runs/YYYY-MM-DD.ndjson`. Downstream readers (`0069`,
`0050`) target `0053`'s. A record written per `0094` is invisible to a reader per
`0053`. **Fix (decide):** either `0094` blesses `0053`'s `looper/telemetry` NDJSON
layout, **or** `0053` layers its index over `0094`'s `.looper/runs/` — pick one.

### H2 · Adapter identity field — `name` (0024) vs `id` (0027). Fix: `0027`→`name`. *(subsumed by B1)*
### H3 · `detect()` input — `workdir: string` (0024) vs injected `RepoFs` (0025/0027/0028, 3-agree). Fix: change **`0024`** to `detect(repo: RepoFs)` + declare `RepoFs` in `@looper/core` (the one HIGH where the owner moves).
...
### H7 · Off-ramp label — `0038` writes `looper:state/needs-human`; canonical `0011` (and 11 other refs) use `looper:needs-human`. Fix: `0038`→`looper:needs-human`.
...
### H11 · `looper prompts` print verb — `show` (0072, CLI owner) vs `render` (0022). Fix: `0022`→`show`.
### H12 · Mode-promotion command — `looper promote <loop> --to` (0009, carries the `tier:core` auto-act guard) vs `looper loops set <loop> mode=` (0071, walkthrough). The `loops set` path bypasses the guard. *(design decision)* **Fix (decide):** make `promote` canonical and `loops set mode=` an alias delegating to it (guard included), or drop `loops set mode=`; update `creating-a-loop.md`.
...
### M3 · `brief_ref` format — `.looper/loops/…@v7` (0012) vs `<loop>/prompt.md@<sha8>` (0022, owner). Fix: `0012`/`0069`/`0072`→`@<sha8>`.
### M4 · `<!-- looper:plan -->` marker — attribute tag (0016, owner) vs open/close pair (0039). Fix: `0039`→`0016`'s self-closing attribute form.
...
### M6 · `looper resume` overloaded — global kill-switch clear (0050) vs per-loop un-pause (0071). Fix: `0050`→`looper resume-all` for the global clear.
...
config-key conflicts (**H8, H9, M1, M2**) all converge on `@looper/config` / `0006`
