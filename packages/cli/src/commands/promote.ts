...
/**
 * `looper promote <loop> --to <mode>` (task 0009): the explicit, audited
...
export function registerPromote(program: Command): void {
  program
    ...
    .argument('<loop>', 'loop name (folder under .looper/loops/)')
    ...
    .action(async (loopName: string, opts: { to: string; path: string }) => {
      ...
      if (!result.ok || !result.config) {
        console.error('config invalid — fix `looper config validate` errors first.');
        process.exitCode = 1;
        return;
      }
      const loop = result.config.loops.find((l) => l.name === loopName);
      if (!loop) {
        console.error(`no loop named '${loopName}' under .looper/loops/`);
        process.exitCode = 2;
        return;
      }
      ...
      const file = join(opts.path, '.looper', 'loops', loopName, 'loop.yml');
