 ...
 * already has and the table in force, compute which looper labels to create.
 * Never plans modification or deletion of labels looper didn't create —
...
export function planLabelReconciliation(
  ...
  const want: LabelSpec[] = [
    ...table.states.map((s) => ({
      name: stateLabel(s),
      color: STATE_COLOR,
      description: `looper lifecycle state: ${s}`,
    })),
    ...OFF_RAMP_LABELS.map((name) => ({
      name,
      color: OFF_RAMP_COLOR,
      description: 'looper off-ramp (terminal/holding)',
    })),
    ...OPERATIONAL_LABELS.map((name) => ({
      name,
      color: OPERATIONAL_COLOR,
      description: 'looper operational marker',
    })),
  ];
