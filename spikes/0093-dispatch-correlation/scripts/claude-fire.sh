...
#
# Required env:
#   LOOPDOG_SPIKE_CLAUDE_FIRE_URL    per-routine /fire URL (imported from Claude web UI)
#   LOOPDOG_SPIKE_CLAUDE_FIRE_TOKEN  per-routine bearer token (imported from Claude web UI)
# Optional env:
...
: "${LOOPDOG_SPIKE_CLAUDE_FIRE_URL:?missing fire URL (import from Claude web UI; see RUNBOOK §1)}"
: "${LOOPDOG_SPIKE_CLAUDE_FIRE_TOKEN:?missing fire token (import from Claude web UI; see RUNBOOK §1)}"
...
HTTP_CODE=$(curl -sS -o "$OUT_DIR/fire-${RUN_ID}.json" -w '%{http_code}' \
  -X POST "$LOOPDOG_SPIKE_CLAUDE_FIRE_URL" \
  -H "Authorization: Bearer $LOOPDOG_SPIKE_CLAUDE_FIRE_TOKEN" \
