...
folded from the run-record telemetry ledger by `looper bench`. Subscription
...
```bash
looper bench --since 30 --out docs/benchmarks.md      # splices between the markers
looper bench --format json --out bench.json           # machine-readable
```
...
- **Source.** The numbers come from the run-record ledger on the `looper/telemetry`
  branch — the same append-only records the runner writes on every transition.
  `projectBenchmark` is pure and deterministic; `looper bench` only reads.
...
mechanism (`looper bench`) is verified offline against synthetic ledgers
